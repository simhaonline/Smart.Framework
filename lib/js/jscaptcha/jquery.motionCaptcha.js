...
/**
 * jQuery MotionCAPTCHA r.20191219
...
jQuery.fn.motionCaptcha || (function($) {
			...
			var touchEndEvent = function(event) {
				...
				if ( !locked && _isDown ) {
					...
					if (_points.length >= 10) {
						...
						if ($canvas.attr('class').match(result.Name) && result.Score >= Number(opts.similarity)) {
							...
							opts.onSuccess($canvas, ctx, result.Score);
	...
	$.fn.motionCaptcha.defaults = {
		...
		shapes: ['triangle', 'x', 'rectangle', 'circle', 'check', 'caret', 'zigzag', 'arrow', 'leftbracket', 'rightbracket', 'v', 'delete', 'leftbrace', 'rightbrace', 'star', 'loop'],
		similarity: 0.75,

		// Canvas vars:
		canvasFont: '13px mono',
		canvasTextColor: '#333333',
		...
		onSuccess: function($canvas, ctx, score) {
			return;
		},
