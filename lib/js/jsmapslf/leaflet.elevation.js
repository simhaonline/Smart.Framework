...
L.Control.Elevation = L.Control.extend({
	...
	//	interpolation: "linear", // no more used with d3v4
		hoverNumber: {
			decimalsX: 3,
			decimalsY: 0,
			formatter: undefined
	...
	onAdd: function(map) {
		...
		var area = this._area = d3.area()
			.curve(d3.curveBasis) //(opts.interpolation)
			.x(function(d) {
	...
	//	if (L.Browser.touch) {
		if (L.Browser.mobile) { // fix by unixman
	...
	/*
	 * Add data to the diagram either from GeoJSON and
