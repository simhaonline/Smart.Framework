...
// file: L.Control.MapCenterCoord.js
...
L.Control.MapCenterCoord = L.Control.extend({
	...
	options: {
		...
		icon: true,
		onMove: false,
		template: 'Center Lat: {y} ; Center Lon: {x}', // https://en.wikipedia.org/wiki/ISO_6709
		projected: false,
	...
	//make a copy of center so we aren't affecting leaflet's internal state
		...
		if (this.options.latlngFormat === 'DM') {
			...
			lng = this._format('#0.0000000', centerCopy.lng); // + 'º';
			lat = this._format('##0.0000000', centerCopy.lat); // + 'º';
		}
