/* jQuery Virtual Keyboard v1.28.7
...
Fixes by unixman (181210):
	- remove dependencies to jQueryUI
...
;(function (factory) {
	...
	//var havejQUi = !!($.ui && $.ui.position); // unixman

	var $keyboard = $.keyboard = function (el, options) {
	var o, base = this;

	base.version = '1.28.7.uxm20190207';
	...
	base.startup = function () {
		...
		/* fix by unixman
		if (o.reposition && havejQUi && o.appendTo === 'body') {
			...
			base.$keyboard.css({ position:'fixed', top:parseInt(position.top + base.$el.height() + 10)+'px', left:parseInt(position.left + (base.$el.width()/2) - (base.$keyboard.width()/2))+'px' });
			//-- #end fix
//		}
	...
	base.buildRow = function ($row, row, keys, acceptedKeys) {
		...
		for (key = 0; key < keys.length; key++) {
			...
			if (/^\{\S+\}$/.test(keys[key])) {
				...
				case 'a':
				case 'accept':
					if(!o.autoAccept) { // unixman: hide accept button if option autoAccept is TRUE
						base
