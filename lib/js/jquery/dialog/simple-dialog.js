...
this.Dialog_Alert = function(y_question_html, evcode, y_title, y_width, y_height) {
	//--
	...
	//--
	$('#simpledialog-area-head').text('' + y_title);
	$('#simpledialog-area-msg').html('' + y_question_html);
	$('#simpledialog-bttn-yes').show().text('').hide();
	$('#simpledialog-bttn-no').show().text('Cancel').click(function () {
		_class.CloseWidget();
...
this.Dialog_Confirm = function(y_question_html, evcode, y_title, y_width, y_height) {
	...
	$('#simpledialog-bttn-yes').show().text('OK').click(function () {
		_class.CloseWidget();
		if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
			try {
				eval('(function(){ ' + evcode + ' })();'); // sandbox
...
$(function() {
	...
	$('body').append('<!-- SmartJS.Modal.Dialog :: r.150527 --><div id="simpledialog-overlay"></div><div id="simpledialog-container"><div id="simpledialog-area-head" class="header">[T]</div><div id="simpledialog-area-msg" class="message">[M]</div><hr><div class="buttons"><div id="simpledialog-bttn-yes" class="yes">[Y]</div><div id="simpledialog-bttn-no" class="no simpledialog-close">[N]</div></div></div><!-- END: SmartJS.Modal.Loader -->');
	//--
}); //END DOCUMENT READY
