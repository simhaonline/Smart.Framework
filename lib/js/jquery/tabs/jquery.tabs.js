...
// REQUIRES-CSS: lib/js/jquery/tabs/jquery.tabs.css
...
this.initTabs = function(tabs_id, prevent_reload, selected) {
	...
	jQuery('#' + tabs_id + ' ul:first').removeAttr('style').addClass('simple_tabs_head').each(function(){
		//-- process tabs
		var jq_exttabs = []; // register external tabs if any
		var jq_links = jQuery(this).find('a'); // find tab header links
		...
		jq_links.each(function(idx, el){
			jQuery(this).removeAttr('style').addClass('simple_tabs_inactive');
			var crrHash = '' + this.hash;
			var CrrHref = '' + this.href;
			if(crrHash) {
				jQuery(crrHash).data('tabs-num', crr_tab).data('tabs-mode', 'internal').removeAttr('style').addClass('simple_tabs_content').hide();
			} else {
	...
	if(jq_content.data('tabs-mode') == 'external') {
		...
		if(jq_content.data('tabs-url') != '') {
			...
			if((jq_content.data('tabs-loaded') != 'yes') || (prevent_reload !== true)) {
				//--
				jq_content.empty().html('<div class="simple_tabs_loader"><img src="' + SmartJS_BrowserUtils_LoaderImg + '" alt="... loading Tab data ..."></div>');
				//--
				setTimeout(function() {
					var ajx = SmartJS_BrowserUtils.Ajax_XHR_Request_From_URL(jq_content.data('tabs-url'), 'GET', 'html');
					ajx.done(function(msg) { // instead of .success() (which is deprecated or removed from newest jQuery)
						jq_content.data('tabs-loaded', 'yes').empty().html('' + msg);
					}).fail(function(msg) { // instead of .error() (which is deprecated or removed from newest jQuery)
						SmartJS_BrowserUtils.alert_Dialog('<h1>WARNING: Asyncronous Load Timeout or URL is broken !</h1>', 'jQuery(\'' + the_hash + '\').empty();', 'TAB #' + ' :: ');
					});
				}, 500);
				//--
			} //end if else
