	...
	HtmlElement.bind('keydown', function(event) {
		...
		source: function(request, response) {
			...
			ajax.done(function(msg) { // {{{JQUERY-AJAX}}}
				...
				alert('UI.AutoCompleteField: FAILED to fetch results for Element: ' + elem_id);
			});
		},
		search: function() {
			// custom minLength
		...
		select: function(event, ui) {
			...
			try {
				...
				alert('UI.AutoCompleteField: ERROR ... could not bind value to Element: ' + elem_id);
			}
			if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
				try {
					eval('(function(){ ' + evcode + ' })();'); // sandbox
				} catch(err) {
					alert('UI.AutoCompleteField ERROR: JS-Eval Error on Element: ' + elem_id + '\nDetails: ' + err);
				} //end try catch
			}
			return false;
		}
