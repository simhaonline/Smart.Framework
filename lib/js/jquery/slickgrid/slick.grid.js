	...
	function SlickGrid(container,data,columns,options) {
		...
		function createColumnHeaders() {
			...
			for (i = 0; i < columns.length; i++) {
				...
				var header = $("<div class='slick-header-column c" + i + " ui-state-default" + (m.cssHeadClass ? ' ' + m.cssHeadClass : '') + "' id='" + uid + m.id + "' />")
...
*/
		...
		function setSelectedRows(rows) {
			...
			// unixman
			var ui_class_selected = "slickgrid-row-selected selected"; // "ui-state-active selected"
		...
		function appendRowHtml(stringArray,row) {
			...
			var cellCss;
			var css = "slick-row " + (dataLoading ? " loading" : "") + (selectedRowsLookup[row] ? " selected slickgrid-row-selected" : "") + (row % 2 == 1 ? ' odd' : ' even'); // unixman: " selected ui-state-active"
