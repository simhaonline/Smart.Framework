 ...
 * All rights reserved.
 *
 * (c) 2012-2019 unix-world.org
 * SlickGrid v1.4.3.1.smart.20190117 : Smart.Framework
 ...
 *			3. 	Default formatter to SmartJS_CoreUtils.escape_html()
...
(function($) {
	...
	function SlickGrid(container,data,columns,options) {
		...
		var gridData, gridDataGetLength, gridDataGetItem;
		...
		function init() {
			...
			$headerScroller = $("<div class='slick-header slick-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
			$headers = $("<div class='slick-header-columns' style='width:100000px; left:-10000px' />").appendTo($headerScroller);
			//--
			$secondaryHeaderScroller = $("<div class='slick-header-secondary slick-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
		...
		/*	var $c = $("<div style='position:absolute; top:-10000px; left:-10000px; width:100px; height:100px; overflow:scroll;'></div>").appendTo("body");
			...
			var $outerdiv = $('<div style="position:absolute; top:-10000px; left:-10000px; overflow:auto; width:100px; height:100px;"></div>').appendTo("body");
		...
		function getMaxSupportedCssHeight() {
			...
			*/
			...
			var testUpTo = 6000000; // unixman: will not support more than Firefox can as Firefox is the etalon browser for all opensource software
			var div = $("<div style='display:none' />").appendTo(document.body);
			while(true) {
		...
		function createColumnHeaders() {
			...
			for(i = 0; i < columns.length; i++) {
				...
				columnsById[m.id] = i;
				var header = $("<div class='slick-header-column c" + i + " slick-state-default" + (m.cssHeadClass ? ' ' + m.cssHeadClass : '') + "' id='" + uid + m.id + "' />")
		...
		function measureCellPaddingAndBorder() {
			var bFix = 1; // fix pixels by unixman (this appears to be a valid fix in Chrome)
			var tmp = $("<div class='slick-state-default slick-header-column' style='visibility:hidden'>-</div>").appendTo($headers);
		...
		function setSelectedRows(rows) {
			...
			// unixman
			var ui_class_selected = "slickgrid-row-selected selected"; // "slick-state-active selected"
			// unselect old rows
		...
		function defaultFormatter(row, cell, value, columnDef, dataContext) {
			...
			return (value === null || value === undefined) ? "" : SmartJS_CoreUtils.escape_html(value);
		} //END FUNCTION
		...
		function appendRowHtml(stringArray,row) {
			...
			var cellCss;
			var css = "slick-row " + (dataLoading ? " loading" : "") + (selectedRowsLookup[row] ? " selected slickgrid-row-selected" : "") + (row % 2 == 1 ? ' odd' : ' even'); // unixman: " selected slick-state-active"
			...
			stringArray.push("<div class='slick-uiwidget-content " + css + "' row='" + row + "' style='top:" + (options.rowHeight*row-offset) + "px'>");
