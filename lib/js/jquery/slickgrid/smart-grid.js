...
var Smart_Grid = function(gridID, infoListID, jsonURL, cookieStorePrefix, sortColumn, sortDir, sortType, formNameID, showLoadNotification, notifCssGrowlClass) { // OBJECT-CLASS
			...
			if((formNameID !== null) && (formNameID != '')) {
				if(msg.hasOwnProperty('filter')) {
					$.each(msg.filter, function(index, value){
						...
						$('#' + formNameID).find(':input[name=' + index + ']').val(value); // https://api.jquery.com/input-selector/
					});
				} //end if
			} //end if
			//--
