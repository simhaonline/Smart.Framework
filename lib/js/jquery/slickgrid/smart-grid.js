...
// DEPENDS: jQuery, jQuery-Growl, jQueryUI, SmartJS_CoreUtils, SmartJS_BrowserUtils, jQuery SlickGrid, jQuery SimplePagination (pager)

var Smart_Grid = function(gridID, infoListID, jsonURL, cookieStorePrefix, sortColumn, sortDir, sortType, formNameID) { // OBJECT-CLASS
...
this.resetGrid = function() {
	//--
	SmartJS_BrowserUtils.deleteCookie(cookieURL, '/');
	//--
} //END FUNCTION
//--
...
this.loadGridData = function(offs) {
	...
	//--
	var the_overlay = SmartJS_BrowserUtils.Overlay_Show('<div align="center"><b>... loading data ...</b></div>');
	//--
	var fdata = '';
	...
	} else if((crrSavedURL === '') && (crrCookieURL !== null)) {
	...
	SmartJS_BrowserUtils.Ajax_XHR_Request_From_URL(crrURL, 'POST', 'json').done(function(msg) { // {{{JQUERY-AJAX}}}
		...
		if((msg.hasOwnProperty('status')) && (msg.status === 'OK') && (msg.hasOwnProperty('rowsList')) && (msg.hasOwnProperty('crrOffset')) && (msg.hasOwnProperty('itemsPerPage')) && (msg.hasOwnProperty('totalRows'))) {
			...
			if((formNameID !== null) && (formNameID != '')) {
				if(msg.hasOwnProperty('filter')) {
					$.each(msg.filter, function(index, value){
						$('#' + formNameID).find('input[name=' + index + ']').val(value);
					});
				} //end if
			} //end if
			//--
			...
			//--
			SmartJS_BrowserUtils.Overlay_Hide(the_overlay);
			//--
		} else {
			//--
