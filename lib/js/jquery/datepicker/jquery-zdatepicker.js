...
// Zebra_DatePicker (SmartLightUI version)

/*
 *  Zebra_DatePicker is a small, compact and highly configurable date picker plugin for jQuery
 *  License: GNU LESSER GENERAL PUBLIC LICENSE
 *
 *  Original Script by: http://stefangabos.ro/jquery/zebra-datepicker/
 ...
 *  current version: 1.9.5.4.smart20190105
 *  Includes fixes by unixman:
 * 		* code runtime blocks optimizations
 * 		* removed support for IE6
 * 		* added support for alternate container
 * 		* replaced jQuery deprecated $.isArray() with Array.isArray()
 * 		* replaced jQuery deprecated delegate() with on()
 * 		* fixed browser as $.browser
 */
;(function(factory) {
	...
	$.Zebra_DatePicker = function(element, options) {
		...
		var init = function(update) {
			...
			// calendar is past only, ending today
				...
				} else if(!tmp_end_date && plugin.settings.direction[1] !== false && Array.isArray(plugin.settings.direction)) { // if have information about the ending date
			...
			if(!plugin.settings.always_visible) {
				...
				if(!update) {
					...
					if(plugin.settings.show_icon) {
						...
						if($.browser.firefox && $element.is('input[type="text"]') && $element.css('display') == 'inline') {
			...
			datepicker.
			//	delegate('td:not(.dp_disabled, .dp_weekend_disabled, .dp_not_in_month, .dp_week_number)', 'mouseover', function() { // unixman: #Fix: delegate() is deprecated since jQuery 3+
				on('mouseover', 'td:not(.dp_disabled, .dp_weekend_disabled, .dp_not_in_month, .dp_week_number)', function() {
					$(this).addClass('dp_hover');
				}).
			//	delegate('td:not(.dp_disabled, .dp_weekend_disabled, .dp_not_in_month, .dp_week_number)', 'mouseout', function() { // unixman: #Fix: delegate() is deprecated since jQuery 3+
			...
			//daypicker.delegate('td:not(.dp_disabled, .dp_weekend_disabled, .dp_not_in_month, .dp_week_number)', 'click', function() { // unixman: #Fix: delegate() is deprecated since jQuery 3+
			daypicker.on('click', 'td:not(.dp_disabled, .dp_weekend_disabled, .dp_not_in_month, .dp_week_number)', function() { // unixman: #Fix: this is the equivalent of delegate() in jQuery 3+

				//console.log('daypicker click delegate() replaced with on()');
			...
			//monthpicker.delegate('td:not(.dp_disabled)', 'click', function() { // unixman: #Fix: delegate() is deprecated since jQuery 3+
			monthpicker.on('click', 'td:not(.dp_disabled)', function() { // unixman: #Fix: this is the equivalent of delegate() in jQuery 3+

				//console.log('monthpicker click delegate() replaced with on()');
			...
			//yearpicker.delegate('td:not(.dp_disabled)', 'click', function() { // unixman: #Fix: delegate() is deprecated since jQuery 3+
			yearpicker.on('click', 'td:not(.dp_disabled)', function() { // unixman: #Fix: this is the equivalent of delegate() in jQuery 3+

				//console.log('yearpicker click delegate() replaced with on()');
		...
		plugin.show = function() {
			...
			if(!plugin.settings.always_visible) {
				...
				if(plugin.settings.container.is('body')) {
					...
					var	window_scroll_top = $(window).scrollTop(), window_scroll_left = $(window).scrollLeft();

						// compute the date picker's default left and top
						// this will be computed relative to the icon's top-right corner (if the calendar icon exists), or
						// relative to the element's top-right corner otherwise, to which the offsets given at initialization
						// are added/subtracted
				//	var	left = (undefined !== icon ? icon.offset().left + icon.outerWidth(true) : $element.offset().left + $element.outerWidth(true)) + plugin.settings.offset[0];
				//	var top = (undefined !== icon ? icon.offset().top : $element.offset().top) - datepicker_height + plugin.settings.offset[1];
					//-- fixes by unixman
					var left = 0;
					var top = 0;
					var theElemTop = 0, theElemHeight = 0;
					var theElemLeft = 0;
					var isAbove = true;
					var theAltElement;
					var theViewportBottom = window_scroll_top + window_height;
					//--
					if(plugin.settings.alternate_container !== false) { // use alternate container for positioning
						theAltElement = $('#' + plugin.settings.alternate_container);
						theElemLeft 	= theAltElement.offset().left;
						theElemTop 		= theAltElement.offset().top;
						theElemHeight 	= theAltElement.outerHeight(true);
					} else {
						theElemLeft 	= $element.offset().left;
						theElemTop 		= $element.offset().top;
						theElemHeight 	= $element.outerHeight(true);
					} //end if else
					//--
					left = theElemLeft + plugin.settings.offset[0];
					//--
					...
					//--
					if(isAbove === false) { // if below
						if(theElemTop > (theViewportBottom - (datepicker_height + plugin.settings.offset[1]))) {
							isAbove = true;
						} //end if
					} else { // if above
						if(theElemTop < (datepicker_height + plugin.settings.offset[1])) {
							isAbove = false;
						} //end if
					} //end if else
					//--
					if(isAbove === false) { // if below
						top = theElemTop + theElemHeight + plugin.settings.offset[1];
					} else { // if above
						top = theElemTop - datepicker_height - plugin.settings.offset[1];
					} //end if else
					//--
						...
						$.each(matches, function(index, match) {
							...
							switch (match.character) {
								...
								case 'M':
									...
									if(match.character == 'D' || match.character == 'l') { // if day is given as day name, we'll check against the names in the used language
										iterable = plugin.settings.days;
									} else { // if month is given as month name, we'll check against the names in the used language
		...
		var disable_text_select = function(el) {
			...
			if($.browser.firefox) { // if browser is Firefox
				el.css('MozUserSelect', 'none');
			} else if($.browser.msie) { // if browser is Internet Explorer
		...
		var format = function(date) {
			...
			for(var i = 0; i < plugin.settings.format.length; i++) {
				...
				switch(chr) {
					...
					case 'M': f = (Array.isArray(plugin.settings.months_abbr) && undefined !== plugin.settings.months_abbr[n - 1] ? plugin.settings.months_abbr[n - 1] : plugin.settings.months[n - 1].substr(0, 3));
					...
					case 'D': l = (Array.isArray(plugin.settings.days_abbr) && undefined !== plugin.settings.days_abbr[w] ? plugin.settings.days_abbr[w] : plugin.settings.days[w].substr(0, 3));
		...
		var generate_daypicker = function() {
			...
			for(var i = 0; i < 7; i++) {
				html += '<th>' + (Array.isArray(plugin.settings.days_abbr) && undefined !== plugin.settings.days_abbr[(plugin.settings.first_day_of_week + i) % 7] ? plugin.settings.days_abbr[(plugin.settings.first_day_of_week + i) % 7] : plugin.settings.days[(plugin.settings.first_day_of_week + i) % 7].substr(0, 2)) + '</th>';
			} //end for
		...
		var generate_monthpicker = function() {
			...
			for(var i = 0; i < 12; i++) {
				...
				html += '<td class="' + $.trim(class_name) + '">' + (Array.isArray(plugin.settings.months_abbr) && undefined !== plugin.settings.months_abbr[i] ? plugin.settings.months_abbr[i] : plugin.settings.months[i].substr(0, 3)) + '</td>';
