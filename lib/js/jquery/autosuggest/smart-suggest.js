...
this.bindToInput = function(y_type, y_id, y_txt, y_res_url, y_use_bttn, y_size, min_term_len, evcode) {
	...
	//--
	var input = jQuery('#' + y_id).attr('autocomplete', 'off').attr('placeholder', y_txt);
	//--
	if(y_type === 'multilist') {
...
this.handleSelection = function(y_type, y_id, id, value, label, evcode) {
	//--
	// for evcode the selected items data is: id (id to display, can be empty, is optional), value (id to select), label (extra description)
	//--
	if(y_type === 'multilist') {
	...
	if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
		try {
			...
			alert('SmartAutoSuggest ERROR: JS-Eval Error on Element: ' + y_id + '\nDetails: ' + err);
		} //end try catch
	} //end if
	//--
	setTimeout(function(){
...
var AutoSuggestShow = function(input, y_size, html) {
	...
	jQuery('#Smart_Suggest_Container').css({
		'position': 'fixed',
		'top': (parseInt(offset.top) + input.outerHeight() + 1) + 'px',
...
var AutoSuggestAction = function(input, y_type, y_size, y_res_url, min_term_len, evcode) {
	//--
	var typedTxt = SmartJS_CoreUtils.arrayGetLast(SmartJS_CoreUtils.stringSplitbyComma(input.val()));
	...
	//--
	var url = '' + y_res_url + encodeURIComponent(typedTxt);
	...
	if(SmartLoaderAutoSuggest != '') {
		theImg = '' + SmartLoaderAutoSuggest;
	} //end if
	AutoSuggestShow(input, y_size, '<div align="center"><img src="' + theImg + '"></div>');
	//--
	setTimeout(function(){
		//--
		jQuery.ajax({
			...
			success: function(answer) {
				...
				for(var i=0; i<(answer.length); i++) {
					//--
					suggest += '<tr valign="top" data-id="' + SmartJS_CoreUtils.escape_html(el) + '" data-value="' + SmartJS_CoreUtils.escape_js(answer[i].value) + '" onclick="SmartAutoSuggest.handleSelection(\'' + SmartJS_CoreUtils.escape_js(y_type) + '\', \'' + SmartJS_CoreUtils.escape_js(theID) + '\', \'' + SmartJS_CoreUtils.escape_js(answer[i].id) + '\', \'' + SmartJS_CoreUtils.escape_js(answer[i].value) + '\', \'' + SmartJS_CoreUtils.escape_js(answer[i].label) + '\', \'' + SmartJS_CoreUtils.escape_js(evcode) + '\');" title="' + SmartJS_CoreUtils.escape_html(el+1) + '">';
					//--
					suggest += '<th>';
...
jQuery(function() {
	...
	jQuery('body').append('<!-- SmartJS.AutoSuggest :: r.160919 --><div id="Smart_Suggest_Container"></div><!-- END: SmartJS.AutoSuggest -->');
	//--
