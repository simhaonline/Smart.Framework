...
(function($) {
	...
	$(function() {
		...
		$("body").append(
			$([
				...
				overlay = $('<div id="lbOverlay">').click(close)[0], // unixman :: this is a fix for jQuery 1.9 or later
		...
		bottom = $('<div id="lbBottom">').appendTo(bottomContainer).append([
			//$('<a id="lbCloseLink" href="#">').add(overlay).click(close)[0], // unixman :: this is a fix for jQuery 1.9 or later
			$('<a id="lbCloseLink" href="#">').click(close)[0], // unixman :: extra feature
	...
	$.slimbox = function(_images, startImage, _options) {
		...
		$(center).css({
			'top': parseInt(Math.max(0, middle - (centerHeight / 2))) + 'px',
			'width': parseInt(centerWidth) + 'px',
			'height': parseInt(centerHeight) + 'px',
			'marginLeft': parseInt(-1 * centerWidth / 2) + 'px'
		}).show();
	...
	*/
	...
	function position() {
		...
		$([center, bottomContainer]).css({
			'left': parseInt(l + (w / 2)) + 'px'
		});
		if(compatibleOverlay) {
			$(overlay).css({
				left: parseInt(l) + 'px',
				top: parseInt(win.scrollTop()) + 'px',
				width: parseInt(w) + 'px',
				height: parseInt(win.height()) + 'px'
			});
		} //end if
	} //END FUNCTION
	...
	function animateBox() {
		...
		//--
		if(the_img_height <= 0) {
			alert('Slideshow: Invalid Image Width !');
		} //end if
		if(the_img_height <= 0) {
			alert('Slideshow: Invalid Image Height !');
		} //end if
		//--
		...
		if((the_img_height > 0) && (the_img_height > the_hmax)) {
			...
			the_img_height = parseInt(the_hmax);
			the_img_width = parseInt(the_img_width * the_img_ratio);
		} //end if
		the_img_height = parseInt(the_img_height);
		//-- resize by width
		var the_wmax = (win.width() - the_offset_left - 5);
		if((the_img_width > 0) && (the_img_width > the_wmax)) {
			the_img_ratio = the_wmax / the_img_width;
			the_img_width = parseInt(the_wmax);
			the_img_height = parseInt(the_img_height * the_img_ratio);
		} //end if
		the_img_width = parseInt(the_img_width);
		//--
		...
		*/
		...
		//--
		var top = parseInt(win.scrollTop()) + 10;
		...
		if(center.offsetWidth != centerWidth) {
			$(center).animate({
				width: centerWidth,
				marginLeft: parseInt(-1 * centerWidth / 2)
			}, options.resizeDuration, options.resizeEasing);
		} //end if
		//--
		$(center).queue(function() {
			$(bottomContainer).css({
				'width': parseInt(centerWidth) + 'px',
				'top': parseInt(top + centerHeight) + 'px',
				'marginLeft': parseInt(-1 * centerWidth / 2) + 'px',
				'visibility': 'hidden',
				'display': ''
			});
	...
	function animateCaption() {
		...
		$(bottom).css({
			'marginTop': parseInt(-1 * bottom.offsetHeight) + 'px'
