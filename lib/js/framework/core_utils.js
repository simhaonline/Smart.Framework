 ...
 * @return {String} The processed string
 */
this.stripslashes = function(str) { // original written by: Kevin van Zonneveld (http://kevin.vanzonneveld.net) ; improved by: Ates Goral, marrtins, rezna ; fixed / bugfixed by: Mick@el, Onno Marsman, Brett Zamir, Rick Waldron, Brant Messenger
...
/*
 * Render Marker Template.
 * This is compatible with the Smart.Framework PHP server-side Markers Templating for substitutions on client-side, except it can only process marker substitutions without extended syntax as IF/LOOP/INCLUDE.
 * Notice: The marker syntax like [####MARKER####] cannot be rendered in a normal view as it will be replaced by server-side with (####+/-MARKER+/-####) to avoid uncontrolled cycles, but in other raw data exchange contexts like json it can be used ...
 *
 ...
 * @return 	{String} 				The processed string
 */
this.render_markers_template = function(template, arrobj) { // v.160212
	//--
...
/*
 * Dump variables like Obects in Javascript
 *
 * Notice: This method can be used OBLY for advanced debugging and never use in production as the browser memory can crash on large objects !
 *
 * @private : debugging only
 * @method print_Object
 * @static
 * @param 	{Object} 	obj 		The object to be dumped
 * @param 	{Enum} 		mode 		*Optional* The operate mode: 'recursive' or undefined
 * @param 	{Enum} 		level 		*Optional* The dump level: '@auto@' : undefined
