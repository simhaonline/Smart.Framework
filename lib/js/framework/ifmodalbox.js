...
var SmartJS_ModalBox_CloseAlign = 'left'; 									// close align: 'left' or 'right'
var SmartJS_ModalBox_vAlign = 'top'; 										// vAlign: 'top' or 'middle' / 'center'
var SmartJS_ModalBox_DelayOpen = 750;										// modal open delay (500 ... 1000) ; default is 750
...
// [PUBLIC]
this.RefreshByEXEC_Parent = function() {
	//--
	if(SmartJS_ModalBox_RefreshBy_State) {
		...
		if((typeof SmartJS_ModalBox_RefreshBy_URL == 'undefined') || (SmartJS_ModalBox_RefreshBy_URL === undefined) || (SmartJS_ModalBox_RefreshBy_URL == '')) {
			//--
			if((typeof parent.location != 'undefined') && (parent.location !== undefined) && (parent.location != '')) { // avoid to reload an empty location
				//parent . location . reload(false); // false is to reload from cache
				parent.location = parent.location; // FIX: avoid reload to resend POST vars !!
			} //end if
			//--
		} else {
			//--
			parent.location = '' + SmartJS_ModalBox_RefreshBy_URL;
			//--
		} //end if else
		//--
...
this.make_Visible = function() {
	//--
	$('#smart__iFModalBox__X').css({
		'visibility': 'visible'
	});
	//--
	$('#smart__iFModalBox__iFrame').css({
	...
	$('#smart__iFModalBox__Ldr').empty().html('');
	//--
	return false;
	//--
} //END FUNCTION
...
var get_Window_Width = function(windowWidth) {
	...
	if((windowWidth <= 0) || isNaN(windowWidth)) {
		//--
		windowWidth = parseInt(parseInt($(window).width()) - 40);
		//--
...
var get_Window_Height = function(windowHeight) {
	...
	if((windowHeight <= 0) || isNaN(windowHeight)) {
		//--
		windowHeight = parseInt(parseInt($(window).height()) - 20);
		//--
...
var do_load = function(yURL, yProtect, windowWidth, windowHeight) {
	...
	//--
	$('#smart__iFModalBox__Ldr').empty().html('<br><br><img src="' + SmartJS_ModalBox_LoaderImg + '" alt="..." title="...">');
	//--
	...
	//--
	$('#smart__iFModalBox__iFrame').show().css({
		'width': '100%',
		'height': '100%',
	...
	if(SmartJS_ModalBox_CloseAlign == 'left') {
		the_align_left = '-17px'; // left
	} else { // right
		the_align_right = '-17px'; // right
	} //end if else
	//--
	$('#smart__iFModalBox__X').show().css({
		'position': 'absolute',
	...
	//--
	if(yProtect != 1) {
		$('#smart__iFModalBox__Bg').click(function(){
			_class.go_UnLoad();
		});
	} else {
		$('#smart__iFModalBox__Bg').unbind('click');
	} //end if
	//-- show delayed
...
var do_unload = function() {
	//--
	$('#smart__iFModalBox__Bg').unbind('click');
	$('#smart__iFModalBox__X').unbind('click');
	$('#smart__iFModalBox__Ldr').empty().html('');
	//--
	var the_align_left = 'auto';
	var the_align_right = 'auto';
	...
	$('#smart__iFModalBox__iFrame').attr('src', '').hide();
	//--
	$('#smart__iFModalBox__Div').css({
		'position': 'absolute',
		'width': '1px',
		'height': '1px',
		'left': '0px',
...
var fx_position = function(windowWidth, windowHeight) {
	//--
	var the_h_align = parseInt(parseInt($(window).scrollLeft()) + ((parseInt($(window).width()) - windowWidth) / 2)) + 'px';
	var the_v_align = parseInt(parseInt($(window).scrollTop()) + 10) + 'px';
	if((SmartJS_ModalBox_vAlign == 'center') || (SmartJS_ModalBox_vAlign == 'middle')) {
		the_v_align = parseInt((parseInt($(window).scrollTop()) + ((parseInt($(window).height()) - windowHeight) / 2))) + 'px';
	} //end if else
	//--
	$('#smart__iFModalBox__Div').css({
		'position': 'absolute',
