...
// v.2.3.7.1 r.2016.08.26 / smart.framework.v.2.3
...
/**
 * Display a page overlay in the current browser window (page)
 *
 * @method Overlay_Show
 * @static
...
this.Overlay_Show = function(text, title, class_name) {
	...
	var overlay = jQuery('<div id="smart_framework_overlay" ' + the_style + '></div>').css({ 'z-index': 2147481901, 'position': 'fixed', 'top': '0px', 'left': '0px', 'width': '100%', 'height': '100%' }).hide().appendTo('body');
...
/**
 * Clear all notifications in a page overlay in the current browser window (page)
 *
...
/**
 * Hide (destroy) the page overlay in the current browser window (page)
 *
 * @method Overlay_Hide
 * @static
 */
 ...
 * @param 	{Boolean} 	sticky 			FALSE by default (will auto-close after the display time expire) ; TRUE to set sticky (require manual close, will not auto-close)
			...
			if((msg.hasOwnProperty('completed')) && (msg.completed == 'DONE') && (msg.hasOwnProperty('status')) && ((msg.status == 'OK') || (msg.status == 'ERROR')) && (msg.hasOwnProperty('action')) && (msg.action != null) && (msg.hasOwnProperty('title')) && (msg.title != null) && (msg.hasOwnProperty('message')) && (msg.message != null) && (msg.hasOwnProperty('redirect')) && (msg.hasOwnProperty('replace_div')) && (msg.hasOwnProperty('replace_html'))) {
				...
				if(msg.status == 'OK') { // OK
					...
					if((msg.redirect != null) && (msg.redirect != '') && (msg.message == '')) {
						...
						if(doReplaceDiv == 'yes') {
							jQuery('#'+msg.replace_div).html(SmartJS_Base64.decode(msg.replace_html));
						} //end if
						if((doReplaceDiv != 'yes') || (msg.message != '')) {
							if(growl === 'yes') {
								Message_AjaxForm_Notification('' + SmartJS_CoreUtils.escape_html(msg.title), '<img src="lib/core/img/sign_info.png" align="right">' + SmartJS_Base64.decode(msg.message), msg.redirect, 'yes', 'gritter-green', 2000);
							} else {
								Message_AjaxForm_Notification('' + SmartJS_CoreUtils.escape_html(msg.action) + ' / ' + SmartJS_CoreUtils.escape_html(msg.title), '<img src="lib/core/img/sign_info.png" align="right">' + SmartJS_Base64.decode(msg.message), msg.redirect, 'no', '', 2000);
							} //end if else
						} //end if else
						if(growl !== 'yes') {
							_class.Overlay_Hide();
					...
					if(growl === 'yes') {
						Message_AjaxForm_Notification('* ' + SmartJS_CoreUtils.escape_html(msg.title), '<img src="lib/core/img/sign_warn.png" align="right">' + SmartJS_Base64.decode(msg.message), msg.redirect, 'yes', 'gritter-red', 3000);
					} else {
						Message_AjaxForm_Notification('* ' + SmartJS_CoreUtils.escape_html(msg.action) + ' / ' + SmartJS_CoreUtils.escape_html(msg.title), '<img src="lib/core/img/sign_warn.png" align="right">' + SmartJS_Base64.decode(msg.message), msg.redirect, 'no', '', 3000);
					} //end if else
					if(growl !== 'yes') {
						_class.Overlay_Hide();
					} //end if
