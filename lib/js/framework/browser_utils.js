...
//======================================= Close a Modal / PopUp Window (and if a parent refresh is pending will execute it)
...
this.Overlay_Show = function(text, title, class_name) {
	...
	var overlay = jQuery('<div id="smart_framework_overlay" ' + the_style + '></div>').hide().appendTo('body').css({ width: jQuery(document).width(), height: jQuery(document).height(), opacity: 0.85 });
	...
	if((typeof text != 'undefined') && (text != null) && (text != '')) {
		...
		GrowlNotificationDoAdd(title, text, '', 750, false, class_name);
	} //end if else
	//--
	return overlay;
	//--
...
this.Overlay_Hide = function(overlay) {
	...
	//--
	jQuery('#smart_framework_overlay').remove(); // remove the instance
	//--
} //END FUNCTION
	...
	if(growl === 'yes') {
		...
		var growlOptions = {
			...
			after_close: function(){
				SmartJS_BrowserUtils.Overlay_Hide(page_overlay);
			}
		};
		//--
...
var GrowlNotificationDoAdd = function(title, text, image, time, sticky, class_name, options) {
	...
	//--
	if(typeof jQuery.gritterx == 'undefined') {
