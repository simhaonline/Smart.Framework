...
this.RequireJS = function(script) {
	//--
	if((typeof script == 'undefined') || (script == null) || (script == '')) {
		console.log('Empty script to load by SmartJS_BrowserUtils.RequireJS');
		return;
	} //end if
	//--
	script = '' + script; // force string
	if(script.split('.').pop() !== 'js') {
		console.log('Invalid script to load by SmartJS_BrowserUtils.RequireJS: ' + script);
		return;
	} //end if
	//--
	if(SmartJS_BrowserUtils_LoadedJScripts.indexOf(script) !== -1) {
		//console.log('Script already loaded by SmartJS_BrowserUtils.RequireJS: ' + script);
		return;
	} //end if
	//--
	SmartJS_BrowserUtils_LoadedJScripts.push(script);
	$('head').append('<script type="text/javascript" src="' + script + '"></script>'); // fix: document.write will fail after document has rendered and it comes from ajax answer
	//--
} //END FUNCTION

//=======================================

this.RequireCSS = function(stylesheet) {
	//--
	if((typeof stylesheet == 'undefined') || (stylesheet == null) || (stylesheet == '')) {
		console.log('Empty stylesheet to load by SmartJS_BrowserUtils.RequireJS');
		return;
	} //end if
	//--
	stylesheet = '' + stylesheet; // force string
	if(stylesheet.split('.').pop() !== 'css') {
		console.log('Invalid stylesheet to load by SmartJS_BrowserUtils.RequireJS: ' + stylesheet);
		return;
	} //end if
	//--
	if(SmartJS_BrowserUtils_LoadedCsStylesheets.indexOf(stylesheet) !== -1) {
		//console.log('Stylesheet already loaded by SmartJS_BrowserUtils.RequireJS: ' + stylesheet);
		return;
	} //end if
	//--
	SmartJS_BrowserUtils_LoadedCsStylesheets.push(stylesheet);
	$('head').append('<link rel="stylesheet" href="' + stylesheet + '">'); // fix: document.write will fail after document has rendered and it comes from ajax answer
	//--
} //END FUNCTION
