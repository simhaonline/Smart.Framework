...
*
* @desc This JavaScript class provides methods simplify the interraction with the Browser and Ajax object access.
...
/**
 * @desc Detect if a Browser Window is iFrame
 *
...
/**
 * @desc Detect if a Browser Window is PopUp
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Parse Current URL to extract GET Params 		Ex: http(s)://some.url/?param1=value1&param2=value%202
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Print current Browser Page
 *
 * @memberof SmartJS_BrowserUtils
 * @method PrintPage
 * @static
...
/**
 * @desc Count Down handler that bind to a HTML Element
 *
 * @memberof SmartJS_BrowserUtils
 * @method CountDown
 * @static
...
/**
 * @desc Page Away control handler. Take control over the events as onbeforeunload to prevent leaving the browser page in unattended mode
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Redirect to URL current browser window
 *
 * @memberof SmartJS_BrowserUtils
 ...
 *
 * @fires Browser will load a new location
 */
...
/**
 * @desc Delayed Redirect to URL current browser window
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Redirect to URL parent browser window
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Lazy Refresh parent browser window or Lazy redirect parent window to another URL.
 * @summary This method is different than RedirectParent() because will be executed just after the child (modal iFrame / PopUp) is closed.
...
/**
 * @desc Close a Modal / PopUp child browser window.
 * @summary If a parent refresh is pending will execute it after.
...
/**
 * @desc Delayed Close a Modal / PopUp child browser window.
 * @summary If a parent refresh is pending will execute it after.
 * @hint This is the prefered way to close a child window (modal iFrame / PopUp) using a button or just executing this code in a child page.
 *
 * @memberof SmartJS_BrowserUtils
...
/*
 * @desc Prevent Modal Cascading overload in browser.
 * @summary If set will use a Popup instead opening a modal iFrame child from another modal iFrame.
...
/**
 * @desc Focus a browser window by reference
 *
 * @memberof SmartJS_BrowserUtils
 * @method windowFocus
 * @static
...
/**
 * @desc Scroll down a browser window by reference.
 * @summary It will focus the referenced browser window first.
 * @hint
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Get the highest available Z-Index from a browser page taking in account all visible layers (div).
 * @summary It will ignore non-visible layers for speed-up, as they have anyway no Z-Index assigned.
 * @hint
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Display a page overlay in the current browser window
 * @summary All page elements that must be visible over this overlay must have zIndex in range: 2147401000 - 214740999
...
/**
 * @desc Clear all notifications from the page overlay in the current browser window
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Hide (destroy) the page overlay in the current browser window
 *
 * @memberof SmartJS_BrowserUtils
 * @method Overlay_Hide
 * @static
...
/**
 * @desc Add a Growl notification in the current browser window (page)
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Remove a Growl Notification by ID from the current browser window (page)
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Create a Maximize HTML Element wrapper arround a layer (div, iframe, ...).
 * @summary This will add a maximize / un-maximize button and the handler to be able to maximize the element.
 * @hint When the selected element will be maximized the page will be protected by an overlay.
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Create a Browser Alert Dialog that will have just the OK button.
 * @summary This will detect if the UIDialog is available and will prefer to use it if set.
 * 			Otherwise will try to detect if SimpleDialog is available and will use it if UIDialog is not available - fallback.
 * @hint If none of the above are available (not available) will try to use jQuery.alertable/alert else display a simple alert() - fallback.
 *
 * @memberof SmartJS_BrowserUtils
 * @method alert_Dialog
 * @static
...
/**
 * @desc Create a Browser Confirm Dialog that will have 2 buttons: OK / Cancel.
 * @summary This will detect if the UIDialog is available and will prefer to use it if set.
 * 			Otherwise will try to detect if SimpleDialog is available and will use it if UIDialog is not available - fallback.
 * @hint If none of the above are available (not available) will try to use jQuery.alertable/confirm else display a simple confirm() - fallback.
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Confirm a Form Submit by a confirm / Dialog, with OK and Cancel buttons.
 * @summary The form will be submitted just if OK button is clicked.
...
/**
 * @desc Open a Modal/iFrame or PopUp child window with a new target to post a form within.
 * @summary It will get the form URL and form method GET/POST directly from the objForm.
...
/**
 * @desc Open a Modal/iFrame or PopUp child window with a new target to open a URL link within.
 * @summary This function can be called by a button, a link or other HTML elements at onClick (if 'a' element onClick is used must be followed by 'return false;' to avoid fire page refresh.
 * @hint
 *
 * @memberof SmartJS_BrowserUtils
 * @method PopUpLink
...
/**
 * @desc Get a Cookie from Browser by Name and return it's Value
 *
 * @memberof SmartJS_BrowserUtils
 * @method getCookie
 * @static
...
/**
 * @desc Set a Cookie in Browser by Name and Value
 *
 * @memberof SmartJS_BrowserUtils
 * @method setCookie
 * @static
...
/**
 * @desc Delete a Cookie from Browser by Name
 *
 * @memberof SmartJS_BrowserUtils
 * @method deleteCookie
 * @static
...
/*
 * @desc Resize iFrames Dinamically on Height and Optional on Width
 *
 * @private : internal development only
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Force a text limit on a TextArea (will also attach a CounterField)
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Catch TAB Key in a TextArea or other compatible input field
 * @summary The input needs this html attribute: onKeyDown="SmartJS_BrowserUtils.catch_TABKey(event);"
 * @hint
 *
 * @memberof SmartJS_BrowserUtils
 * @method catch_TABKey
...
/**
 * @desc Check or Uncheck all checkboxes in a form element
 *
 * @memberof SmartJS_BrowserUtils
...
/*
 * @desc Background Send (post) a Form (it does not catch the result, just send it to ensure updates in some cases ...).
 * @summary This should be used for very particular situations by example posting a form to post another form before !!
...
/**
 * @desc Submit a Form by Ajax via POST and Handle the Answer
 * @summary This function expects a json answer with the following structure: see more in framework PHP lib SmartComponents::js_ajax_replyto_html_form()
 * @example
...
/**
 * @desc Create an Ajax XHR Request (POST) by Form
 * @summary It supports simple forms or complex forms with multipart/form-data and file attachments.
 * @hint
 *
 * @memberof SmartJS_BrowserUtils
...
/*
 * @desc Create an Ajax XHR Request (POST) using multipart/form-data type to bse used with file attachments.
 * @summary Instead using this directly is better to use:
...
/**
 * @desc Create a general purpose Ajax XHR Request (GET/POST) with Optional support for Authentication and Extra Headers
 * @summary For creating an Ajax XHR Request to be used with HTML Forms use:
 * 		SmartJS_BrowserUtils.Ajax_XHR_GetByForm(); 		// basic, will detect the form type if must use multipart/form-data + attachments (if any)
 * or even much better use this:
 * 		SmartJS_BrowserUtils.Submit_Form_By_Ajax(); 	// advanced, will handle the XHR form request and the answer
...
/**
 * @desc Loads the contents for a Div (or other compatible) HTML Element(s) by Ajax using a GET / POST Ajax Request
...
/**
 * @desc Add (Create) a Bookmark to Favorites in Browser
...
/**
 * @desc Trigger a function when you scroll to an element
 *
 * @memberof SmartJS_BrowserUtils
 * @method WayPoint
 * @static
...
/**
 * @desc Create a virtual file download from Javascript
 *
 * @memberof SmartJS_BrowserUtils
...
/**
 * @desc Create a virtual image upload handler
 * @summary Requires in HTML:
...
/*
 * @desc Set Refresh on PopUp Parent
 *
 * @private
 *
 * @memberof SmartJS_BrowserUtils
