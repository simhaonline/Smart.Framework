...
this.param_Notifications = 'growl'; // growl | dialog (default notification to be used)
this.param_NotifyLoadError = false; // if set to false will not raise notifications on ajax load errors: true | false
this.param_Use_iFModalBox_Active = 0;
this.param_Use_iFModalBox_Protection = 0;
this.param_PopUp_ShowToolBar = 0;
this.param_FullScreen_Img = 'lib/js/framework/img/fullscreen-on.svg';
this.param_Cloner_Img_Add = 'lib/js/framework/img/clone-insert.svg';
this.param_Cloner_Img_Remove = 'lib/js/framework/img/clone-remove.svg';
...
this.param_DefaultCloseDelayedTimeout = 750;
this.param_Time_Notification_OK = 1000;
this.param_Time_Notification_ERR = 3000;
this.param_Time_Delay_Redirect = 1500;
 ...
 * @example 	Ex: http(s)://some.url/?param1=value1&param2=value%202
 *
 * @memberof SmartJS_BrowserUtils
...
this.CloneElement = function(elem, destination, elType, maxLimit) {
	...
	//--
	jQuery('#' + destination).append('<span id="' + 'clone_container__' + SmartJS_CoreUtils.escape_html(elem) + '_' + SmartJS_CoreUtils.escape_html(uuID) + '"><br><img alt="Remove" title="Remove" height="16" src="' + SmartJS_CoreUtils.escape_html(SmartJS_BrowserUtils.param_Cloner_Img_Remove) + '" style="cursor:pointer; vertical-align:middle;" onClick="jQuery(this).parent().remove();">&nbsp;&nbsp;</span>');
	//--
	switch(elType) {
		case 'text-input':
		case 'text-area':
...
var init_PopUp = function(strUrl, strTarget, windowWidth, windowHeight, forcePopUp, forceDims) {
	...
	if(((typeof SmartJS_ModalBox != 'undefined') && (SmartJS_BrowserUtils.param_Use_iFModalBox_Active) && (forcePopUp != 1)) || (forcePopUp == -1)) { // use smart modal box
		...
		try {
			SmartJS_BrowserUtils.param_PopUpWindow = window.open(strUrl, strTarget, "top=" + windowTop + ",left=" + windowLeft + ",width=" + windowWidth + ",height=" + windowHeight + ",toolbar="+SmartJS_BrowserUtils.param_PopUp_ShowToolBar+",scrollbars=1,resizable=1");
		} catch(err){
