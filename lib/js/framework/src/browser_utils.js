...
*/
var SmartJS_BrowserUtils = new function() { // START CLASS :: v.170906.r3
...
this.PopUpSendForm = function(objForm, strTarget, windowWidth, windowHeight, forcePopUp, forceDims, evcode) {
	...
	try {
		...
		alert('SmartJS_BrowserUtils.PopUpSendForm :: ERROR: Invalid Form Object');
		return;
	} //end try catch
				...
				if(msg.status == 'OK') { // OK
					...
					if((msg.redirect != null) && (msg.redirect != '') && (msg.message == '')) {
						...
						if((doReplaceDiv != 'yes') || (msg.message != '')) {
							if(growl === 'yes') {
								Message_AjaxForm_Notification(SmartJS_CoreUtils.escape_html(String(msg.title)), '<img src="' + SmartJS_BrowserUtils_ImgOK + '" align="right">' + msg.message, msg.redirect, 'yes', 'gritter-green', 2000);
							} else {
								Message_AjaxForm_Notification(SmartJS_CoreUtils.escape_html(String(msg.action)) + ' / ' + SmartJS_CoreUtils.escape_html(msg.title), '<img src="' + SmartJS_BrowserUtils_ImgOK + '" align="right">' + msg.message, msg.redirect, 'no', '', 2000);
							} //end if else
						} //end if else
						if(growl !== 'yes') {
							_class.Overlay_Hide();
...
var Message_AjaxForm_Notification = function(ytitle, ymessage, yredirect, growl, class_growl, timeout) {
	...
	if(growl === 'yes') {
		...
		if((typeof yredirect != 'undefined') && (yredirect != null) && (yredirect != '')) {
			_class.RedirectDelayedToURL(yredirect, (timeout + 1500)); // comment this to disable override of mouse over growl
			redirectAfterClose = function(){
				SmartJS_BrowserUtils.RedirectDelayedToURL(String(yredirect), 500);
			};
		} else {
			setTimeout(function(){ SmartJS_BrowserUtils.Overlay_Hide(); }, (timeout + 1500)); // comment this to disable override of mouse over growl
		} //end if
		//--
		var growlOptions = {
