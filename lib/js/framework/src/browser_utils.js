...
*/
var SmartJS_BrowserUtils = new function() { // START CLASS :: v.20190213.r9
...
this.PrintPage = function() {
	...
	try {
		...
		console.error('Printing Check N/A: ' + err);
		alert('WARNING: Printing may not be available in your browser');
	} //end try catch
	//--
} //END FUNCTION
	...
	if(typeof SmartJS_BrowserUIUtils != 'undefined') { // use UI Dialog (the best choice)
		...
		if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
			try {
				...
				console.error('ERROR: JS-Eval Error on Browser DialogAlert Function' + '\nDetails: ' + err);
			} //end try catch
		} //end if
		//--
	} //end if else
		...
		if(the_confirmation) {
			if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
				try {
					...
					console.error('ERROR: JS-Eval Error on Browser DialogConfirm Function' + '\nDetails: ' + err);
				} //end try catch
			} //end if
		} //end if
		//--
	...
	if((the_form_id == null) || (the_form_id == '')) {
		...
		if(found_files !== true) {
			...
			try {
				...
				alert('ERROR: Ajax_XHR_GetByForm / FormData Object Failed. File Attachments NOT sent ! Try to upgrade / change your browser. Your browser does not support HTML5 File Uploads.'); // this must be alert to anounce the user
				data = jQuery('#' + the_form_id).serialize(); // no files detected use serialize
				ajax = _class.Ajax_XHR_Request_From_URL(url, 'POST', data_type, data);
 ...
 * @param 	{Decimal} 	imgMaxResizeMBSize 			The max resized image size in MB (0.01 ... 1) ; Default is 0.1
 ...
 */
this.VirtualImageUploadHandler = function(inputUpldFileId, previewPlaceholderId, imgQuality, imgMaxResizeMBSize, imgMaxResizeW, imgMaxResizeH, fxDone, clearPreview) {
	//--
	if(!inputUpldFileId) {
	...
	} else if(imgMaxResizeMBSize > 1) {
		imgMaxResizeMBSize = 1; // max 1MB
	} //end if else
	...
	$upldr.on('change', function() {
		...
		if(the_size_of_file < 43) { // check file size: min JPEG size is 134 bytes ; min PNG size is unknown ; min GIF size is 43 bytes
			...
			return;
		} else if(the_size_of_file > (1024 * 1024 * 4)) { // check uploaded max file size (<4MB)
			$upldr.val('');
			$imgpw.text('Image Uploader WARNING: Invalid File Size - Image is larger than ' + imgMaxMBSize + 'MB');
			return;
		} //end if
		//--
		$imgpw.html('<div title="Remove the Image" style="width:20px; height:20px; line-height:20px; font-size:15px; font-weight:bold; color:#111111; cursor:pointer;" onClick="jQuery(\'#' + SmartJS_CoreUtils.escape_html(inputUpldFileId) + '\').val(\'\'); jQuery(\'#' + SmartJS_CoreUtils.escape_html(previewPlaceholderId) + '\').html(\'\');">&times;</div>');
		//--
		try {
			//--
				...
				setTimeout(function(){
						...
						if(isSVG) {
							if(String(the_frd.result).length <= (1024 * 1024 * imgMaxResizeMBSize)) {
								if(typeof fxDone == 'function') {
									try {
										fxDone(String(the_frd.result), w, h, true, String(the_type_of_file), String(the_frd.result).length);
									} catch(e){
								...
								$upldr.val('');
								$imgpw.text('Image Uploader WARNING: SVG Size is higher than allowed: ' + String(the_frd.result).length + ' Bytes');
							} //end if else
							if(clearPreview) {
								$imgpw.empty().html('');
							} //end if
							...
							im.onload = function(){
								...
								if(String(imgResizedB64).length <= (1024 * 1024 * imgMaxResizeMBSize)) {
									...
									$upldr.val('');
									$imgpw.text('Image Uploader WARNING: Image Size after resize is higher than allowed: ' + String(imgResizedB64).length + ' Bytes');
								} //end if else
							};
