...
var SmartJS_BrowserUtils_NotifyLoadError = true; // if set to false will not raise notifications on ajax load errors
			...
			if(counter > 0) {
				...
				if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
					try {
						...
						console.log('ERROR: JS-Eval Error on Browser CountDown Function' + '\nDetails: ' + err);
					} //end try catch
				} //end if
				//--
			} //end if
...
this.RedirectParent = function(yURL) {
	...
	if((typeof yURL == 'undefined') || (yURL == null) || (yURL == '')) {
		console.log('WARNING: Parent Redirection to Empty URL is not allowed !');
		return;
	} //end if
	//--
	...
	if(typeof SmartJS_BrowserUIUtils != 'undefined') { // use UI Dialog (the best choice)
		...
		if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
			try {
				...
				console.log('ERROR: JS-Eval Error on Browser DialogAlert Function' + '\nDetails: ' + err);
			} //end try catch
		} //end if
		//--
	} //end if else
		...
		if(the_confirmation) {
			if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
				try {
					...
					console.log('ERROR: JS-Eval Error on Browser DialogConfirm Function' + '\nDetails: ' + err);
				} //end try catch
			} //end if
		} //end if
		//--
...
this.confirmSubmitForm = function(y_confirm, y_form, y_target, windowWidth, windowHeight, forcePopUp, forceDims) {
	...
	if((typeof y_form == 'undefined') || (y_form == null) || (y_form == '')) {
		//--
		console.log('ERROR: Form Object is Undefined in confirmSubmitForm()');
		//--
	} else {
		//--
...
this.PopUpSendForm = function(objForm, strTarget, windowWidth, windowHeight, forcePopUp, forceDims, evcode) {
	...
	//--
	if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
		setTimeout(function(){ objForm.submit(); try { if(typeof evcode === 'function') { evcode(objForm, strTarget, forcePopUp, forceDims); } else { eval(evcode); } } catch(err) { console.log('ERROR: PopUpSendForm: After-Form JS Code Err: ' + err); } }, 500); // delay submit for buggy browsers
	} else {
...
this.textArea_Limit = function(y_field, y_countfield, y_maxlimit) {
	...
	//--
	//console.log('Counting for Area: ' + y_field + ' ; using CounterField: ' + y_countfield);
...
this.textArea_addLimit = function(y_field, y_maxlimit) {
	...
	//--
	var y_countfield = '__LimitedTextArea__' + y_field + '__Fld_COUNTER__';
	//--
	jQuery('#' + y_field).wrap(
		'<div style="position:relative!important;"></div>'
	).after(
		'<div style="position:relative!important; text-align:left!important;"><input title="TextArea Limit: ' + SmartJS_CoreUtils.escape_html(y_maxlimit) + '" type="text" readonly disabled id="' + SmartJS_CoreUtils.escape_html(y_countfield) + '" size="6" maxlength="5" value="' + SmartJS_CoreUtils.escape_html(y_maxlimit) + '" style="cursor:help!important; padding:0px!important; font-size:10px!important; text-align:center!important; position:relative!important; margin-top:-7px!important; opacity:0.75;"></div>'
	).on('change click blur keydown keyup paste', function(){
...
this.Background_Send_a_Form = function(other_form_id, evcode) {
	...
	if(ajax === null) {
		console.log('ERROR: Background Submit Form / Null XHR Object !');
		return;
	} //end if
	//--
	_class.Overlay_Show();
	...
	ajax.done(function(msg) { // {{{JQUERY-AJAX}}}
		...
		if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
			try {
				...
				console.log('ERROR: JS-Eval Error on Background Send Form' + '\nDetails: ' + err);
			} //end try catch
		} //end if
		//--
		_class.Overlay_Hide();
...
this.Submit_Form_By_Ajax = function(the_form_id, url, growl, evcode, everrcode) {
	...
	if(ajax === null) {
		console.log('ERROR: Submit Form by Ajax / Null XHR Object !');
		return;
	} //end if
	//--
	_class.Overlay_Show();
				...
				if(msg.status == 'OK') { // OK
					...
					if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
						try {
							...
							console.log('ERROR: JS-Eval Error on Submit Form By Ajax (1)' + '\nDetails: ' + err);
						} //end try catch
					} //end if
					//--
					if((msg.js_evcode != null) && (msg.js_evcode != '')) {
						try {
							...
							console.log('ERROR: Msg-JS-Eval Error on Submit Form By Ajax (1)' + '\nDetails: ' + err);
						} //end try catch
					} //end if
					//--
					...
					if((typeof everrcode != 'undefined') && (everrcode != 'undefined') && (everrcode != null) && (everrcode != '')) {
						try {
							...
							console.log('ERROR: JS-Eval Error on Submit Form By Ajax (2)' + '\nDetails: ' + err);
						} //end try catch
					} //end if
					//--
					if((msg.js_evcode != null) && (msg.js_evcode != '')) {
						try {
							...
							console.log('ERROR: Msg-JS-Eval Error on Submit Form By Ajax (2)' + '\nDetails: ' + err);
						} //end try catch
					} //end if
					//--
					if(growl === 'yes') {
...
this.Ajax_XHR_GetByForm = function(the_form_id, url, data_type) {
	...
	//--
	if((typeof url == 'undefined') || (url == null) || (url == '')) {
		console.log('ERROR: Empty URL for Ajax_XHR_GetByForm ...');
		return null;
	} //end if
	//--
	if((the_form_id == null) || (the_form_id == '')) {
		...
		if(found_files !== true) {
			...
			try {
				...
				console.log('ERROR: Ajax_XHR_GetByForm / Invalid FormID !');
				return null;
			} //end try catch
			//--
			try {
...
this.WayPoint = function(targetElem, evcode) {
	...
	var theElement = jQuery('' + targetElem);
	//--
		...
		if(wS > (hT+hH-wH)){
			...
			if((typeof evcode != 'undefined') && (evcode != 'undefined') && (evcode != null) && (evcode != '')) {
				try {
					...
					console.log('ERROR: JS-Eval Error on Browser WayPoint Function' + '\nDetails: ' + err);
				} //end try catch
			} //end if
		} //end if
	});
...
var GrowlNotificationDoAdd = function(title, text, image, time, sticky, class_name, options) {
	...
	if(typeof jQuery.gritter == 'undefined') {
		...
		alert(title + '\n' + text); // if growl notif fails, use the alert !
...
var init_PopUp = function(strUrl, strTarget, windowWidth, windowHeight, forcePopUp, forceDims) {
	...
	if(((typeof SmartJS_ModalBox != 'undefined') && (SmartJS_BrowserUtils_Use_iFModalBox_Active) && (forcePopUp != 1)) || (forcePopUp == -1)) { // use smart modal box
		...
		try {
			...
			console.log('ERROR when trying to raise a new PopUp Window: ' + err);
		} //end try catch
		if(SmartJS_BrowserUtils_PopUpWindow) {
			try { // post-focus
