...
this.textArea_addLimit = function(y_field, y_maxlimit) {
	...
	jQuery('#' + y_field).on('change click blur keydown keyup paste', function(){
		...
		//--
		field.attr('title', '# Text Area Limit: ' + y_maxlimit + ' ; Remaining Characters: ' + (y_maxlimit - field.val().length) + ' #'); // update the counter
		//--
	}).attr('maxlength', y_maxlimit).attr('title', '# Text Area Limit: ' + y_maxlimit + ' #');
	//--
} //END FUNCTION
