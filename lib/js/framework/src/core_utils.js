...
*/
var SmartJS_CoreUtils = new function() { // START CLASS :: v.180330
 ...
 * @return 	{String} 				The processed string
 */
this.render_markers_template = function(template, arrobj, urlescaped) { // v.180330
	//--
	var debug = false;
	//--
	if((typeof template === 'string') && (typeof arrobj === 'object')) {
		...
		if(template != '') {
			//--
			var regexp = /\[####([A-Z0-9_\-\.]+)(\|bool|\|int|\|dec[1-4]{1}|\|num|\|htmid|\|jsvar|\|json|\|substr[0-9]{1,5}|\|subtxt[0-9]{1,5})?(\|url)?(\|js|\|html)?(\|html|\|js)?(\|nl2br|\|url)?(\|url|\|js)?(\|js|\|url)?####\]/g; // {{{SYNC-REGEX-MARKER-TEMPLATES}}}
			//--
