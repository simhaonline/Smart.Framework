...
*/
var SmartJS_CoreUtils = new function() { // START CLASS :: v.181123
...
/**
 * Render Simple Marker Template + Comments (only marker replacements with escaping or processing syntax and support for comments and special replacements: SPACE, TAB, R, N ; no support for IF / LOOP / INCLUDE syntax since the js regex is too simplistic and this can be implemented using real js code)
 ...
 * @return 	{String} 				The processed string
 */
this.render_markers_template = function(template, arrobj, urlescaped) { // v.181123
	//--
	var debug = false;
	//--
	if((typeof template === 'string') && (typeof arrobj === 'object')) {
		...
		if(template != '') {
			//--
			var regexp = /\[####([A-Z0-9_\-\.]+)(\|bool|\|int|\|dec[1-4]{1}|\|num|\|htmid|\|jsvar|\|json|\|lower|\|upper|\|ucfirst|\|ucwords|\|trim|\|substr[0-9]{1,5}|\|subtxt[0-9]{1,5})?(\|url)?(\|js|\|html)?(\|html|\|js)?(\|nl2br|\|url)?(\|url|\|js)?(\|js|\|url)?####\]/g; // {{{SYNC-REGEX-MARKER-TEMPLATES}}}
			//--
		...
		//		} //end if
				...
				var tmp_match_1 		= marker[2] ? String(marker[2]) : ''; 	// |bool |int |num |htmid |jsvar |json |lower |upper |ucfirst |ucwords |trim |substr12345 |subtxt67890
				var tmp_match_2 		= marker[3] ? String(marker[3]) : ''; 	// |url
				var tmp_match_3 		= marker[4] ? String(marker[4]) : ''; 	// |js |html
				var tmp_match_4 		= marker[5] ? String(marker[5]) : ''; 	// |html |js
				var tmp_match_5 		= marker[6] ? String(marker[6]) : ''; 	// |nl2br |url
				var tmp_match_6 		= marker[7] ? String(marker[7]) : ''; 	// |url |js
				var tmp_match_7 		= marker[8] ? String(marker[8]) : ''; 	// |js |url
				//--
				if((tmp_marker_id != null) && (tmp_marker_id != '') && (tmp_marker_key != null) && (tmp_marker_key != '') && (template.indexOf(tmp_marker_id) >= 0)) { // indexOf() :: if not found returns -1 # check if exists because it does replaceAll on a cycle so another cycle can run without scope !
					...
					if(tmp_marker_key in arrobj) {
						...
						if(tmp_match_1 == '|bool') { // Boolean
						...
						} else if(tmp_match_1.substring(0, 4) == '|dec') { // Dec (1..4)
							var xnum = parseInt(tmp_match_1.substring(4));
							...
							tmp_marker_val = String(tmp_marker_val);
							if(debug) {
								console.log('Marker Format Dec[' + xnum + ']: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #1 ' + tmp_match_1 + ' @ ' + tmp_marker_val);
							} //end if
							xnum = null;
						...
						} else if(tmp_match_1 == '|lower') { // apply lowercase
							tmp_marker_val = String(tmp_marker_val).toLowerCase();
							if(debug) {
								console.log('Marker Format Apply LowerCase: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #1 ' + tmp_match_1 + ' @ ' + tmp_marker_val);
							} //end if
						} else if(tmp_match_1 == '|upper') { // apply uppercase
							tmp_marker_val = String(tmp_marker_val).toUpperCase();
							if(debug) {
								console.log('Marker Format Apply UpperCase: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #1 ' + tmp_match_1 + ' @ ' + tmp_marker_val);
							} //end if
						} else if(tmp_match_1 == '|ucfirst') { // apply uppercase first character
							tmp_marker_val = String(_class.stringUcFirst(tmp_marker_val));
							if(debug) {
								console.log('Marker Format Apply UcFirst: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #1 ' + tmp_match_1 + ' @ ' + tmp_marker_val);
							} //end if
						} else if(tmp_match_1 == '|ucwords') { // apply uppercase on each word
							tmp_marker_val = String(_class.stringUcWords(tmp_marker_val));
							if(debug) {
								console.log('Marker Format Apply UcWords: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #1 ' + tmp_match_1 + ' @ ' + tmp_marker_val);
							} //end if
						} else if(tmp_match_1 == '|trim') { // apply trim
							tmp_marker_val = String(_class.stringTrim(tmp_marker_val));
							if(debug) {
								console.log('Marker Format Apply Trim: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #1 ' + tmp_match_1 + ' @ ' + tmp_marker_val);
							} //end if
						...
						if(tmp_match_6 == '|url') {
							tmp_marker_val = _class.escape_url(tmp_marker_val);
							if(debug) {
								console.log('Marker URL-Escape: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #6 ' + tmp_match_6 + ' @ ' + tmp_marker_val);
							} //end if
						} else if(tmp_match_6 == '|js') {
							tmp_marker_val = _class.escape_js(tmp_marker_val);
							if(debug) {
								console.log('Marker JS-Escape: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #6 ' + tmp_match_6 + ' @ ' + tmp_marker_val);
							} //end if
						} //end if else
						//-- #7 Escape JS / URL
						if(tmp_match_7 == '|js') {
							tmp_marker_val = _class.escape_js(tmp_marker_val);
							if(debug) {
								console.log('Marker JS-Escape: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #7 ' + tmp_match_7 + ' @ ' + tmp_marker_val);
							} //end if
						} else if(tmp_match_7 == '|url') {
							tmp_marker_val = _class.escape_url(tmp_marker_val);
							if(debug) {
								console.log('Marker URL-Escape: ' + tmp_marker_id + ' :: ' + tmp_marker_key + ' #7 ' + tmp_match_7 + ' @ ' + tmp_marker_val);
							} //end if
						} //end if else
						//--
