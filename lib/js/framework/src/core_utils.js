 ...
 */
this.stringRegexMatchAll = function(str, regexp) { // v.170428
	//--
	if((typeof str == 'undefined') || (str == undefined) || (str == null)) {
		str = '';
	} else {
	...
	//--
	var matches = [];
	var arguments = {}; // fix: required init to avoid using using arguments from a global context !
 ...
 * @return 	{String} 				The processed string
 */
this.render_markers_template = function(template, arrobj, urlescaped) { // v.170430
	//--
	var debug = false;
	//--
	if((typeof template === 'string') && (typeof arrobj === 'object')) {
		...
		//		} //end if
				...
				var tmp_marker_id 		= marker[0] ? marker[0].toString() : ''; 	// [####THE-MARKER|escapings...####]
				var tmp_marker_key 		= marker[1] ? marker[1].toString() : ''; 	// THE-MARKER
				var tmp_match_1 		= marker[2] ? marker[2].toString() : ''; 	// |bool |int |num |htmid |jsvar |json |substr12345 |subtxt67890
				var tmp_match_2 		= marker[3] ? marker[3].toString() : ''; 	// |url
				var tmp_match_3 		= marker[4] ? marker[4].toString() : ''; 	// |js
				var tmp_match_4 		= marker[5] ? marker[5].toString() : ''; 	// |html
				var tmp_match_5 		= marker[6] ? marker[6].toString() : ''; 	// |nl2br
				//--
				if((tmp_marker_id != null) && (tmp_marker_id != '') && (tmp_marker_key != null) && (tmp_marker_key != '') && (template.indexOf(tmp_marker_id) >= 0)) { // indexOf() :: if not found returns -1 # check if exists because it does replaceAll on a cycle so another cycle can run without scope !
					...
					if(tmp_marker_key in arrobj) {
						...
						tmp_marker_val = '' + tmp_marker_val.toString();
			...
			//	} //end if
				...
				for(var i=0; i<undef_markers.length; i++) {
					...
					var tmp_undef_marker_id = undef_marker[0] ? undef_marker[0].toString() : ''; // [####THE-MARKER|escapings...####]
					undef_log += tmp_undef_marker_id + '\n';
				} //end for
				console.error('WARNING: SmartJS/Core: {#### Undefined Markers detected in Template: ^' + '\n' + undef_log + '\n' + template + '$ #####}');
				delete undef_log;
				delete undef_marker;
				delete undef_markers;
			} //end if
			//--
			if(template.indexOf('[%%%%') >= 0) { // indexOf() :: if not found returns -1
	...
	//--
	return '' + template.toString(); // fix to return empty string instead of null [OK]
	//--
} //END FUNCTION
