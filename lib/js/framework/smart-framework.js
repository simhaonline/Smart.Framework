// JS Combined: smart-framework.js from lib/js/framework/src/*.js @ rev: svn.853
...
/*
 * Render Simple Marker Template (only marker replacements, no IF or LOOP or marker escaping syntax)
 ...
 * Notice: It ONLY supports the marker syntax like [####MARKER####] ; It cannot be used together with the server-side templating as it will have the markers replaced by (####+/-MARKER+/-####)
 *
 * @private : internal use only (when no server-side PHP is available to render a simple markers template)
 ...
 * @return 	{String} 				The processed string
 */
this.render_markers_template = function(template, arrobj) { // v.170308
	//--
...
// [LIB - SmartFramework / JS / Archive Utils]
// (c) 2006-2017 unix-world.org - all rights reserved
// v.2.3.7.7 r.2017.02.22 / smart.framework.v.2.3
...
this.decompressFromBase64 = function(input) {
	...
	//--
	return _class.decompressRawLZS(output);
	//--
} //END FUNCTION
...
this.compressRawLZS = function(uncompressed) {
	...
	//--
	return RawDeflate('' + arch + '#CHECKSUM-SHA1#' + SmartJS_CryptoHash.sha1(arch));
	//--
} //END FUNCTION
...
this.decompressRawLZS = function(compressed) {
	...
	//--
	var unarch = '' + SmartJS_CoreUtils.stringTrim(RawInflate(''+compressed));
	var parts = unarch.split('#CHECKSUM-SHA1#');
	unarch = SmartJS_CoreUtils.stringTrim('' + parts[0]);
	var checksum = SmartJS_CoreUtils.stringTrim('' + parts[1]);
	//-- aaa
	if(SmartJS_CryptoHash.sha1(unarch) !== ('' + checksum)) {
		alert('JS-LZS Archiver / Decompress: Checksum Failed'); // do not raise error just alert
		return '';
	} //end if
	//--
	return SmartJS_CoreUtils.hex2bin(unarch.toLowerCase());
	//--
} //END FUNCTION
...
// [LIB - SmartFramework / JS / Crypto Utils]
// (c) 2006-2017 unix-world.org - all rights reserved
// v.2.3.7.7 r.2017.02.22 / smart.framework.v.2.3

// DEPENDS: SmartJS_CoreUtils
// CONTAINS: Base64 encode/decode ; CryptoHash: MD5, SHA1, SHA512 ; Blowfish encrypt/decrypt
...
this.unicodeString = function() {
	return 'Unicode String:		şŞţŢăĂîÎâÂșȘțȚ 05-9#';
} //END FUNCTION

this.testBase64 = function() {
	return 'VW5pY29kZSBTdHJpbmc6CQnFn8WexaPFosSDxILDrsOOw6LDgsiZyJjIm8iaIDA1LTkj';
} //END FUNCTION

this.testMD5 = function() {
	return '3e16bfa755d156e02dc7d19cb6720849';
} //END FUNCTION

this.testSHA1 = function() {
	return '7f6589d102fb06317663d138b97c87b9cfea0f06';
} //END FUNCTION

this.testSHA512 = function() {
	return '0492494c9afa3fbf88a4fa31fe1af23be75d616f39f8de51eb545a86b5f564ff1a8676ef6c125d7ba63a966d6d1a55fd0ca5d17ab26ababaa6f0dd57eaa3f302';
} //END FUNCTION

this.raiseError = function(err) {
	alert('ERROR: SmartJS/Crypto: Your browser FAILED with: ' + err);
...
var base64_core_enc = function(input) {
	//-- make it unicode
...
var base64_core_dec = function(input) {
	//-- decoder
	var keyStr = base64_key();
	var output = "";
	var chr1, chr2, chr3;
	...
	//--
	return SmartJS_CoreUtils.utf8_decode(output); // make it back unicode safe
	//--
} //END FUNCTION
...
/**
* CLASS :: Hash Crypto: MD5 / SHA1 / SHA512
*
...
var setIV = function(key) {
	...
	var data = '' + SmartJS_CryptoHash.sha1('@SmartFrameworkCrypto/BlowFish:' + key + '#' + SmartJS_CryptoHash.sha1('BlowFish-iv-SHA1' + key) + '-' + SmartJS_CryptoHash.md5('BlowFish-iv-MD5' + key) + '#').substring(29, (8+29));
	//alert('iV: ' + data);
	//-- pre-process
...
var setKey = function(key) {
	...
	r_key = '' + SmartJS_CryptoHash.sha512(key).substring(13, (29+13)) + SmartJS_CryptoHash.sha1(key).substring(13, (10+13)) + SmartJS_CryptoHash.md5(key).substring(13, (9+13));
	//alert('Key: ' + r_key);
	//--
	return r_key;
	//--
...
this.encrypt = function(plaintext, key) {
	...
	plaintext = '' + plaintext + '#CHECKSUM-SHA1#' + SmartJS_CryptoHash.sha1(plaintext);
	//--
	...
	//-- HEX
	return array_map(cipher, function(item){ return (item<=0xf?'0':'')+item.toString(16); }).join("").toUpperCase(); // HEX
	//--
	//=====
	//--
} //END FUNCTION
...
this.decrypt = function(ciphertext, key) {
	...
	//--
	var plaintext = SmartJS_CoreUtils.stringTrim('' + array_map(pt, function(item){ return String.fromCharCode(item); }).join(""));
	//=====
	//-- {{{SYNC-BLOWFISH-CHECKSUM}}}
	var parts = plaintext.split('#CHECKSUM-SHA1#');
	plaintext = SmartJS_CoreUtils.stringTrim('' + parts[0]);
	var checksum = SmartJS_CoreUtils.stringTrim('' + parts[1]);
	if(SmartJS_CryptoHash.sha1(plaintext) !== ('' + checksum)) {
		alert('JS-Blowfish / Decode: Checksum Failed'); // do not raise error just alert
		return '';
	} //end if
	//-- convert to string and reverse b64 :: b64 is because is not UTF-8 safe and may corrupt unicode characters
	return SmartJS_Base64.decode(plaintext); // string
	//--
} //END FUNCTION
...
/**
 * Force text limit on TextArea with attached CounterField
 *
...
// #END# JS Combined: smart-framework.js from lib/js/framework/
