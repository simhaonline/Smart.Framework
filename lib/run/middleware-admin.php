...
define('SMART_FRAMEWORK_RELEASE_MIDDLEWARE', '[A]@v.2.3.3.2');
...
public static function Run() {
	...
	if((string)$winmod != '') { // modal template
		if(isset($appSettings['template-modal-popup-file'])) {
			...
			$the_template_file = Smart::safe_filename(trim((string)$configs['app']['admin-template-modal-popup-file'])); // use default modal template
		} //end if else
	} else { // main template
