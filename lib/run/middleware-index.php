...
define('SMART_FRAMEWORK_RELEASE_MIDDLEWARE', '[I]@v.2.3.1.7');
...
public static function Run() {
	...
	if(strlen(trim((string)$_COOKIE['SmartApp_IDX_LANGUAGE_SET'])) > 0) {
		SmartTextTranslations::setLanguage(trim((string)$_COOKIE['SmartApp_IDX_LANGUAGE_SET']));
	} //end if
	...
	if(class_exists('SmartAppAdminController')) {
		if(((string)get_parent_class('SmartAppIndexController') != 'SmartAbstractAppController') AND (((string)get_parent_class('SmartAppIndexController') != 'SmartAppAdminController') OR ((string)get_parent_class('SmartAppAdminController') != 'SmartAbstractAppController'))) {
			self::Raise500Error('Invalid Module Class Inheritances for Page (2): '.$page);
			return;
		} //end if
	} else {
		if((string)get_parent_class('SmartAppIndexController') != 'SmartAbstractAppController') {
	...
	//== RUN THE MODULE
	//--
	$appModule = new SmartAppIndexController($the_path_to_module, $base_script, $base_full_path, $base_full_url, $page, $the_controller_name);
	//--
